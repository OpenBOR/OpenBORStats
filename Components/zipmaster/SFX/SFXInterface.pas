...
const
  // 'DZ'#0#1;  // current header signature
  SFX_HEADER_SIG = $01005A44;
  // 'DZ'#10#1; // current end of header signature
  ...
  TSFXFileHeader = packed record
    ...
    CmdLine: string;    if CmdLineSize > 0, contains a command to execute after extraction completed
                        ...
                        + CmdLine has a special format:
                          all occurences of  "><" (greather than+less than) in this string will be replaced by
...
implementation
  ...
  - eventually one or more strings (depending on the headers' properties) (not #0 terminated)
