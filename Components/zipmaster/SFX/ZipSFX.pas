(*
  ...
  @author((C) 2002 markus stephany, merkes@mirkes.de, all rights reserved.)
...
type
  ...
  protected
    ...
    *)
    ...
    (* options which control the behaviour of the generated sfx:<br><br>
       ...
       the SFX window, so that list cannot be shown.<br><br>
       ...
       or the "x" button on the title bar.<br><br><br><br>
    *)
    property Options: TSFXOptions read FOptions write FOptions default [];
    (* This value controls the behaviour of the SFX when a file to extract would be overwrite an existing
       file on disk:<br><br>
    ...
    *)
    ...
    (* This string will be stored in the created self extracting archive (SFX).<br>
       ...
       - Each occurence of &gt;&lt; (greater than + less than characters) in this string will be replaced by the
       ...
       - If soExpandVariables is set, a special environment variable %TICKS% is supported, it returns the current
         internal timer counter, so rather unique extraction pathes can be created.<br><br>

       - A special value &ltVOLATILE&gt; is also supported. if this "path" is beeing used as default extraction path,
         it'll be expanded to a unique directory under the temp path. if files have been extracted to this unique
         dir AND a command line has been successfully executed, this directory will be deleted recursively nex time
    ...
    *)
    ...
    (* This string will be stored in the created self extracting archive (SFX).<br>
       ...
       - This string may consist of two parts separated by a pipe ("|") character. Then the part in front
         ...
         application.<br>
       - Each occurence of &gt;&lt; (greater than + less than characters) in this string will be replaced by
    ...
    *)
    ...
    (* If the the file in TargetFile already exists on converting, the existing file will be deleted.
  ...
  public
    ...
    *)
    ...
    (* The self extracting EXE archive in SourceFile will be converted back to a regular ZIP archive archive whose
...
begin
  ...
  FSFXPath := 'DZSFX'+FSFXLanguage+'.BIN';
...
begin
  ...
  then
    ...
    then
      ...
      // not yet localized
      raise EDelphiZip.CreateDisp('Can only convert ZIP to EXE or EXE to ZIP', True);
end;
...
begin
  ...
  if not FileExists(FSourceFile) and (not bCreateNew)
  then
    raise EDelphiZip.CreateResDisp(GE_NoZipSpecified, True);
  ...
  then
    try
      ...
      begin
        ...
        if Pos('DZSFX;', srID) = 1 then
        begin
          Delete(srID, 1, Length('DZSFX;'));
          if Pos(';', srID) > 1 then
          begin
...
begin
  ...
  hFindSFX := FindResource(HInstance, PChar('DZSFX'+SLanguage+'_BIN'), RT_RCDATA);
  if hFindSFX <> 0 then
  begin
    hFindSize := FindResource(HInstance, PChar('DZSFX'+SLanguage+'_BIN_SIZE'), RT_RCDATA);
  ...
  begin
    ...
    begin
      SetString(FSFXPath, pSearch, SearchPath(nil,
        PChar('DZSFX'+SLanguage+'.BIN'), nil, sizeof(pSearch), pSearch, PChar(pData)));
    end;
    if not FileExists(FSFXPath) then
    begin
      SetString(FSFXPath, pSearch, SearchPath(nil,
        PChar('DZSFX'+DZSFX_LANG+'.BIN'), nil, sizeof(pSearch), pSearch, PChar(pData)));
    end;
...
begin
  ...
  try
    ...
    if not (GetNewHeaderProperties or GetOldHeaderProperties)
    then
      raise EDelphiZip.CreateDisp('Unknown SFX header format in '+sFile, True);
...
begin
  ...
  FEOC.HeaderSig := EndCentralDirSig;
  Position := 0;
end;
...
begin
  ...
  begin
    FEOC.CentralOffSet := cNewSize;
    str.CopyFrom(self, sizeof(FEOC)-iPos);
  end
  else
  begin
    cPosOfEOC := ReadEndOfCentral(self, FEOC);
    iDelta := cPosOfEOC - (FEOC.CentralOffset + FEOC.CentralSize);
    Position := iPos;
    // copy local data
    if ((FEOC.CentralOffSet + iDelta) -iPos) > 0
    then
      str.CopyFrom(self, (FEOC.CentralOffSet + iDelta) -iPos);
    // copy and adjust central structures
    CopyAndAdjustCentralStructures(self, str, FEOC, iDelta + cNewSize -iPos);
  end;
end;
...
begin
  ...
  sr := AnsiUpperCase(sr);
  if Pos('DZSFX', sr) = 1 then
  begin
    if Pos('_BIN_LANG'#0, sr) > 0 then
    begin
      sr1 := '';
      ...
      sr := Copy(sr, Pos('DZSFX', sr)+5, MaxInt);
      Delete(sr, Pos('_BIN_LANG', sr), MaxInt);
      if sr1 = ''
      then
